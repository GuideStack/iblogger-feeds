...
```mermaid
graph TD;
    A["Problem: Rat Infestation"] --> B["Policy: Cash Bounty for Dead Rats"];
    B --> C["Intended Effect: Citizens Hunt Wild Rats"];
    C --> D["Expected Outcome: Fewer Rats"];
    B --> E["Unintended Effect: Citizens Breed Rats for Profit"];
    E --> F["Policy Canceled"];
    F --> G["Actual Outcome: More Rats Released (Cobra Effect)"];
