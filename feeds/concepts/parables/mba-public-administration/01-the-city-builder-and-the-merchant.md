...
```mermaid
graph TD;
    A["Public Sector (Government)"] -->|Focuses on| B["Public Good & Equity"];
    C["Private Sector (Business)"] -->|Focuses on| D["Profit & Efficiency"];
    B --> E["Public-Private Partnership (PPP)"];
    D --> E;
    E --> F["Infrastructure Development"];
    E --> G["Risk Sharing"];
    F --> H["Economic Growth"];
    G --> H;
```
