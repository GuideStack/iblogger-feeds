...
| # | សំណួរ (Why?) | ចម្លើយ (Answer) |
|---|---|---|
| 1 | ហេតុអ្វីបានជា App មាន Bug ច្រើនធ្លាក់ដល់ដៃ User? <br/>(Why did so many bugs reach users?) | ពីព្រោះមានការរុញ Bug ទៅវិញទៅមក (Ping-Pong) ឥតឈប់ឈររវាង Developer និង QA ក្រោមសម្ពាធពេលវេលាដ៏ខ្លី ធ្វើឱ្យការតេស្តចុងក្រោយមុន Release មិនបានហ្មត់ចត់។ <br/>(Because there was an endless "ping-pong" cycle of bugs going back and forth between Dev and QA under tight time pressure, leading to an incomplete test sweep at release.) |
| 2 | ហេតុអ្វីបានជាមានការរុញ Bug ទៅវិញទៅមកក្រោមសម្ពាធខ្លាំងបែបនេះ? <br/>(Why was there a rushed ping-pong cycle?) | ពីព្រោះ Developer ត្រូវបង្ខំចិត្តសរសេរកូដយ៉ាងប្រញាប់ប្រញាល់បំផុត ដើម្បីប្រគល់ឱ្យ QA ឱ្យទាន់កាលកំណត់ ដែលនាំឱ្យកូដដំបូងមាន Bug ច្រើន។ <br/>(Because developers had to rush coding to deliver to QA on time, shipping buggy code just to meet the handoff deadline.) |
| 3 | ហេតុអ្វីបានជា Developer ត្រូវសរសេរកូដប្រញាប់ប្រញាល់ខ្លាំងបែបនេះ? <br/>(Why did developers have to rush coding?) | ពីព្រោះពេលវេលាសម្រាប់សរសេរកូដត្រូវបានរួញខ្លី ដើម្បីទុកពេលបញ្ចូលក្រុម QA ផ្ទៃក្នុង ដោយមិនបានពន្យារពេលសរុបនៃគម្រោងឡើយ។ <br/>(Because the coding timeline was squeezed to fit internal QA testing, without extending the overall project schedule.) |
| 4 | ហេតុអ្វីបានជាពេលវេលាសរសេរកូដត្រូវរួញខ្លី ដោយមិនបានពន្យារពេលគម្រោងសរុប? <br/>(Why was the coding timeline squeezed?) | ពីព្រោះការប៉ាន់ស្មានពេលដំបូងមិនបានគិតគូរពីម៉ោង QA ឡើយ (ព្រោះអតិថិជនមិនព្រមបង់ថ្លៃ)។ ប៉ុន្តែថ្នាក់ដឹកនាំនៅតែបន្ថែម QA ផ្ទៃក្នុងដើម្បីធានាគុណភាព ដោយមិនបានកែសម្រួល Timeline គម្រោងឡើងវិញ។ <br/>(Because the initial estimate had no QA hours as the client refused to pay. However, management added internal QA anyway without adjusting the overall project timeline.) |
| 5 | ហេតុអ្វីបានជាការប៉ាន់ស្មានគម្រោងដំបូងមិនបានបញ្ចូលម៉ោង QA? <br/>(Why did the initial estimate exclude QA?) | **ពីព្រោះអតិថិជនបដិសេធមិនព្រមបង់ថ្លៃសេវា QA ហើយផ្នែកលក់ (Sales) ក៏យល់ព្រមកាត់វាចេញពីគម្រោងដំបូងដើម្បីដណ្តើមយកគម្រោងនេះឱ្យបាន រួចសង្ឃឹមថានឹងប្រើប្រាស់ QA ផ្ទៃក្នុងជាក្រោយ។** <br/>(**Because the client refused to pay for QA, and Sales accepted this cut just to win the project, planning to force internal QA into the same squeezed schedule later.**) |
...
graph TD
    A["🔴 បញ្ហា៖ App មាន Bug ច្រើនធ្លាក់ដល់ដៃ User<br/>(Problem: App shipped with too many bugs)"] --> B["Why 1៖ ការរុញ Bug ទៅវិញទៅមក (Ping-Pong) រវាង Dev និង QA<br/>(Endless bugs ping-pong between Dev & QA under pressure)"]
    B --> C["Why 2៖ Dev ត្រូវប្រញាប់សរសេរកូដដើម្បីប្រគល់ឱ្យ QA ទាន់ពេល<br/>(Devs rushed coding to meet handoff date, shipping buggy code)"]
    C --> D["Why 3៖ ពេលវេលាសរសេរកូដត្រូវរួញខ្លីដើម្បីទុកពេលបញ្ចូល QA ផ្ទៃក្នុង<br/>(Coding phase squeezed to insert internal QA without extending timeline)"]
    D --> E["Why 4៖ ការប៉ាន់ស្មានដំបូងគ្មានម៉ោង QA ព្រោះ Client មិនបង់ថ្លៃ<br/>(Initial estimate had no QA hours because client refused to pay)"]
